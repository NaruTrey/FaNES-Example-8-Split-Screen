...
do
	...
	cc65 $cFile -Oir --static-locals --standard c99 -o "$asmFile" -I includes/ -g
	ca65 $asmFile -o build/`basename -s .c $cFile`.o -I includes/
done

for asmFile in `find . -type f -name "*.s"`
do
	ca65 $asmFile -o build/`basename -s .s $asmFile`.o -I includes/
done
